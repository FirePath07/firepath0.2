      ...
      <div className={`transition-all duration-300 ${isTutorialMode ? 'pointer-events-none' : ''}`}>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              ...
              <div className="space-y-6">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <p className="text-gray-700 font-medium">FIRE Number (25x Future Expenses)</p>
            ...
            <div className={`bg-white rounded-xl shadow-lg p-8 relative transition-transform duration-300 ${isTutorialMode ? 'z-50 scale-105 ring-4 ring-emerald-400 pointer-events-auto' : ''}`}>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Quick Actions</h2>
              <div className="grid grid-cols-2 gap-4">
                <button
                  onClick={() => navigate('/risk')}
                  className="p-4 border border-emerald-100 rounded-xl hover:bg-emerald-50 transition text-left group"
                >
                  <span className="text-2xl mb-2 block">🛡️</span>
                  <span className="font-bold text-gray-900 group-hover:text-emerald-700">Risk Profile</span>
                  <p className="text-xs text-gray-500 mt-1">Review & update risk</p>
                </button>

                <button
                  onClick={() => navigate('/calculators')}
                  className="p-4 border border-blue-100 rounded-xl hover:bg-blue-50 transition text-left group"
                >
                  <span className="text-2xl mb-2 block">🧮</span>
                  <span className="font-bold text-gray-900 group-hover:text-blue-700">Calculators</span>
                  <p className="text-xs text-gray-500 mt-1">Plan your future</p>
                </button>

                <button
                  onClick={() => navigate('/articles')}
                  className="p-4 border border-amber-100 rounded-xl hover:bg-amber-50 transition text-left group"
                >
                  <span className="text-2xl mb-2 block">📚</span>
                  <span className="font-bold text-gray-900 group-hover:text-amber-700">Learn</span>
                  <p className="text-xs text-gray-500 mt-1">Financial guides</p>
                </button>

                <button
                  onClick={() => navigate('/questionnaire')}
                  className="p-4 border border-purple-100 rounded-xl hover:bg-purple-50 transition text-left group"
                >
                  <span className="text-2xl mb-2 block">📝</span>
                  <span className="font-bold text-gray-900 group-hover:text-purple-700">Retake</span>
                  <p className="text-xs text-gray-500 mt-1">Update assessment</p>
                </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-8">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <span className="text-xl font-bold text-amber-700">{formatIndianCurrency(financialData.monthlyIncome - financialData.monthlyExpenses)}</span>
                </div>
